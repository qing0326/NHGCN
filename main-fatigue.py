...
from prepare_data_fatigue import *
import argparse
...
if __name__ == '__main__':
    ...
    ######## Data ########
    parser.add_argument('--dataset', type=str, default='fatigue')
    parser.add_argument('--data-path', type=str, default='/data0/meya/code/cqq/LGGNet/dataset/fatigue')
    ...
    parser.add_argument('--save-path', default='./save_fatigue/')
    parser.add_argument('--load-path', default='./save_fatigue/max-acc.pth')
    parser.add_argument('--load-path-final', default='./save_fatigue/final_model.pth')
    ...
    #Preprocess data
    pd = PrepareData_fatigue(args)
    pd.run(sub_to_run)

    #Training model
    cv = CrossValidation(args)
    seed_all(args.random_seed)
    if args.train=='dependent': #对象内
        cv.dependent_n_fold_CV(sub_to_run)
    elif args.train=='independent':  #跨对象
